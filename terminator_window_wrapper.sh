...
/usr/bin/terminator_old ${@:+$@} &

# Wait for the application to start
sleep 2

# Get the window ID of the most recently opened Terminator window
WINDOW_ID=$(wmctrl -l | awk 'BEGIN{found}; /\/bin\/bash/ {found=$1}; END{print found}')
